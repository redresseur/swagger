...
func {{$method.Name}}(ctx *gin.Context){
    ...
    {{else}}
    {{$param.Name}} := ctx.Query("{{$param.Key}}")
    {{end}}{{end}}{{ if  (eq $param.IN  "path") }}{{$param.Name}} := ctx.Param("{{$param.Key}}"){{end}}{{ if  (eq $param.IN  "query") }}{{$param.Name}} := ctx.Param("{{$param.Key}}"){{end}}{{end}}
